...
namespace Project_Lykos
{
    public class ProcessTask
    {
        ...
        public readonly Data.QueueEntry? DbEntry;
        ...
        public ProcessTask(Data.QueueEntry dbRecord)
        {
            DbEntry = dbRecord;
            // Set essential parameters
            WavSourcePath = dbRecord.WavSourcePath;
            LipOutputPath = dbRecord.LipOutputPath;
            UUID = dbRecord.UUID;
            // Remove any quotes from text
            Text = dbRecord.Text.Replace("\"", "");
            UseNativeResampler = dbRecord.UseNativeResampler.GetValueOrDefault();
            if (String.IsNullOrWhiteSpace(dbRecord.WavTempPath))
            {
                WavTempPath = GetTempWavPath(WavSourcePath);
                dbRecord.WavTempPath = WavTempPath;
            }
            else
                WavTempPath = dbRecord.WavTempPath;
        }
        ...
        public async Task<int> ProcessAsync(CancellationToken controlToken, TimeSpan timeOut)
        {
            ...
            try
            {
                ...
                if (DbEntry != null)
                {
                    DbEntry.ExitCode = task.ExitCode;
                    DbEntry.Runtime = task.RunTime;
                    DbEntry.Output = task.StandardOutput;
                    DbEntry.ErrorMessage = task.StandardError;
            ...
            catch (TaskCanceledException e)
            {
                ...
                {
                    DbEntry.ExitCode = -1;
                    DbEntry.Runtime = null;
                    DbEntry.Output = e.ToString();
                    DbEntry.ErrorCount = DbEntry.ErrorCount.GetValueOrDefault() + 1;
                    DbEntry.ErrorMessage = e.Message;
                }
        ...
        public string GetCommand()
        {
            ...
            if (DbEntry != null && DbEntry.Command != output)
                DbEntry.Command = output;
            return output;
        }
    }
}
